/**
 * ============================================================
 * API слой для работы с Supabase
 * ============================================================
 * 
 * Версия: 2.2.0 - ИСПРАВЛЕНО
 * Дата обновления: 2026-02-01
 * 
 * ИЗМЕНЕНИЯ v2.2:
 * ✅ Добавлены методы insertProduct() и insertCategory()
 * ✅ Исправлена проблема с ID overflow (Date.now() слишком большой)
 * ✅ Улучшенная обработка ошибок в syncAll()
 * ✅ Понятные сообщения для пользователя
 * ✅ Возврат userMessage для UI
 * ✅ Не выбрасывает исключение при частичном успехе
 * 
 * @version 2.2.0
 * @author Migration Team
 * @date 2026-02-01
 * ============================================================
 */

import { createClient } from '@supabase/supabase-js'
import { bulkUpdateProducts } from './bulkOperations'
import { delay } from '../utils/helpers'
import { log, warn, error, isDev } from '../utils/logger'

// ============================================================
// КОНФИГУРАЦИЯ SUPABASE
// ============================================================

const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL
const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY

// Проверка наличия переменных окружения
const isEnvConfigured = !!(SUPABASE_URL && SUPABASE_ANON_KEY)

if (!isEnvConfigured) {
  error('❌ Ошибка: Необходимо настроить переменные окружения')
  error('   VITE_SUPABASE_URL и VITE_SUPABASE_ANON_KEY')
}

/**
 * ============================================================
 * КЛИЕНТ SUPABASE
 * ============================================================
 *
 * Создаем клиент для взаимодействия с Supabase
 * Настройки аутентификации:
 * - persistSession: сохраняет сессию в localStorage
 * - autoRefreshToken: автоматически обновляет токен
 * - detectSessionInUrl: определяет сессию из URL (для OAuth)
 *
 * Если переменные окружения не настроены — возвращаем null,
 * чтобы не крашить модуль и показать страницу настройки.
 */
export const supabaseClient = isEnvConfigured
  ? createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
      auth: {
        persistSession: true,
        autoRefreshToken: true,
        detectSessionInUrl: true
      }
    })
  : null

/**
 * ============================================================
 * ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ
 * ============================================================
 */

/**
 * ============================================================
 * КЛАСС API
 * ============================================================
 * 
 * Основной класс для работы с Supabase API
 * Инкапсулирует всю логику работы с базой данных
 */
class SupabaseAPI {
  constructor() {
    this.client = supabaseClient
  }

  /**
   * ============================================================
   * ПОЛУЧИТЬ СЛЕДУЮЩИЙ order_index
   * ============================================================
   *
   * Запрашивает MAX(order_index) из таблицы и возвращает MAX + 1.
   * Это единственный надёжный способ: не зависит от того,
   * сколько элементов сейчас в массиве на клиенте.
   *
   * Проблема length+1:
   *   Было 5 категорий (1,2,3,4,5) → удалили 4 и 5 → length = 3
   *   Новая категория: order_index = 3+1 = 4
   *   Ещё одна:        order_index = 3+1 = 4  ← дубликат! Сортировка сломана.
   *
   * Решение MAX+1:
   *   MAX в базе = 3 → следующий = 4 ✅
   *   Снова MAX  = 4 → следующий = 5 ✅
   *
   * @param {'products'|'categories'} table - Таблица
   * @param {number|null} categoryId - Только для products: считает индекс
   *                                   внутри категории, а не глобально
   * @returns {Promise<number>} Следующий order_index
   */
  async getNextOrderIndex(table, categoryId = null) {
    let query = this.client
      .from(table)
      .select('order_index')
      .order('order_index', { ascending: false })
      .limit(1)

    if (table === 'products' && categoryId !== null) {
      query = query.eq('category_id', categoryId)
    }

    const { data, error: qError } = await query

    if (qError) {
      error(`❌ Ошибка получения max order_index (${table}):`, qError)
      return 9999
    }

    const maxIndex = data?.[0]?.order_index ?? 0
    return maxIndex + 1
  }

  /**
   * ============================================================
   * ЗАГРУЗКА КАТЕГОРИЙ
   * ============================================================
   * 
   * Получает все категории из базы данных
   * Сортирует по order_index для правильного отображения
   * 
   * @returns {Promise<Array>} Массив объектов категорий
   * @throws {Error} Выбрасывает ошибку если загрузка не удалась
   */
  async fetchCategories() {
    // Проверка инициализации клиента
    if (!this.client) {
      throw new Error('Supabase клиент не инициализирован')
    }

    log('📦 Загрузка категорий...')
    
    // Выполняем запрос к таблице categories
    const { data, error: qError } = await this.client
      .from('categories')
      .select('*')
      .order('order_index')

    // Обработка ошибок
    if (qError) {
      error('❌ Ошибка загрузки категорий:', qError)
      throw qError
    }
    
    log(`✅ Загружено ${data.length} категорий`)
    return data
  }

  /**
   * ============================================================
   * ЗАГРУЗКА ПРОДУКТОВ
   * ============================================================
   * 
   * Получает все продукты из базы данных
   * Сортирует по order_index (null значения в конце)
   * 
   * @returns {Promise<Array>} Массив объектов продуктов
   * @throws {Error} Выбрасывает ошибку если загрузка не удалась
   */
  async fetchProducts() {
    // Проверка инициализации клиента
    if (!this.client) {
      throw new Error('Supabase клиент не инициализирован')
    }

    log('📦 Загрузка продуктов...')
    
    // Выполняем запрос к таблице products
    // nullsLast: true - продукты без order_index в конце списка
    const { data, error: qError } = await this.client
      .from('products')
      .select('*')
      .order('order_index', { nullsLast: true })

    // Обработка ошибок
    if (qError) {
      error('❌ Ошибка загрузки продуктов:', qError)
      throw qError
    }
    
    log(`✅ Загружено ${data.length} продуктов`)
    return data
  }

  /**
   * ============================================================
   * ВСТАВКА НОВОГО ПРОДУКТА
   * ============================================================
   * 
   * Создает новый продукт в базе данных
   * PostgreSQL автоматически генерирует ID через SERIAL
   * Это решает проблему с Date.now() (слишком большой ID)
   * 
   * @param {Object} productData - Данные нового продукта
   * @param {number} productData.category_id - ID категории
   * @param {string} productData.name - Название продукта
   * @param {string} productData.volume - Объем (например, "750ml")
   * @param {number} productData.bar1 - Остаток на баре 1
   * @param {number} productData.bar2 - Остаток на баре 2
   * @param {number} productData.cold_room - Остаток в холодильной комнате
   * @param {number} [productData.order_index] - Индекс для сортировки (опционально)
   * 
   * @returns {Promise<Object>} Созданный продукт с автоматически сгенерированным ID
   * @throws {Error} Выбрасывает ошибку если создание не удалось
   */
  async insertProduct(productData) {
    if (!this.client) {
      throw new Error('Supabase клиент не инициализирован')
    }

    log('➕ Создание нового продукта:', productData.name)

    try {
      // Считаем order_index прямо из базы: MAX по этой категории + 1
      // Это защищает от дубликатов при удалениях (см. getNextOrderIndex)
      const orderIndex = await this.getNextOrderIndex('products', productData.category_id)

      const { data, error: insertError } = await this.client
        .from('products')
        .insert([{
          category_id: productData.category_id,
          name: productData.name,
          volume: productData.volume,
          bar1: productData.bar1 ?? 0,
          bar2: productData.bar2 ?? 0,
          cold_room: productData.cold_room ?? 0,
          order_index: orderIndex,
          is_frozen: false,
          visible_to_bar1: true,
          visible_to_bar2: true
        }])
        .select()
        .single()

      if (insertError) {
        error('❌ Ошибка создания продукта:', insertError)
        throw insertError
      }

      log('✅ Продукт создан с ID:', data.id, '| order_index:', orderIndex)
      return data

    } catch (err) {
      error('❌ Критическая ошибка вставки продукта:', err)
      throw err
    }
  }

  /**
   * ============================================================
   * ВСТАВКА НОВОЙ КАТЕГОРИИ
   * ============================================================
   * 
   * Создает новую категорию в базе данных
   * PostgreSQL автоматически генерирует ID через SERIAL
   * Это решает проблему с Date.now() (слишком большой ID)
   * 
   * @param {Object} categoryData - Данные новой категории
   * @param {string} categoryData.name - Название категории
   * @param {number} categoryData.order_index - Индекс для сортировки
   * 
   * @returns {Promise<Object>} Созданная категория с автоматически сгенерированным ID
   * @throws {Error} Выбрасывает ошибку если создание не удалось
   */
  async insertCategory(categoryData) {
    if (!this.client) {
      throw new Error('Supabase клиент не инициализирован')
    }

    log('➕ Создание новой категории:', categoryData.name)

    try {
      // Считаем order_index прямо из базы: MAX по всем категориям + 1
      // Это защищает от дубликатов при удалениях (см. getNextOrderIndex)
      const orderIndex = await this.getNextOrderIndex('categories')

      const { data, error: insertError } = await this.client
        .from('categories')
        .insert([{
          name: categoryData.name,
          order_index: orderIndex
        }])
        .select()
        .single()

      if (insertError) {
        error('❌ Ошибка создания категории:', insertError)
        throw insertError
      }

      log('✅ Категория создана с ID:', data.id, '| order_index:', orderIndex)
      return data

    } catch (err) {
      error('❌ Критическая ошибка вставки категории:', err)
      throw err
    }
  }

  /**
   * ============================================================
   * ОБНОВЛЕНИЕ ОДНОГО ПРОДУКТА
   * ============================================================
   * 
   * Обновляет остатки для одного продукта
   * Используется для точечных обновлений
   * Для массовых операций используйте syncAll()
   * 
   * Включает retry механизм для сетевых ошибок:
   * - При ошибке пытается повторить запрос до 2 раз
   * - С увеличивающейся задержкой (1 сек, 2 сек)
   * 
   * @param {number} productId - ID продукта для обновления
   * @param {Object} updates - Объект с полями для обновления
   * @param {number} [updates.bar1] - Новое значение остатка бар 1
   * @param {number} [updates.bar2] - Новое значение остатка бар 2
   * @param {number} [updates.cold_room] - Новое значение остатка холодной комнаты
   * @param {number} [retryCount=0] - Счетчик попыток (используется внутренне)
   * 
   * @returns {Promise<Object>} Результат операции {success, productId, error?}
   */
  async updateProductStock(productId, updates, retryCount = 0) {
    // Проверка инициализации клиента
    if (!this.client) {
      throw new Error('Supabase клиент не инициализирован')
    }

    try {
      // Выполняем UPDATE для конкретного продукта
      const { error: updateError } = await this.client
        .from('products')
        .update(updates)
        .eq('id', productId)

      // Если есть ошибка - выбрасываем
      if (updateError) throw updateError

      // Успешное обновление
      return { success: true, productId }

    } catch (err) {
      // Retry логика для сетевых ошибок
      // Проверяем: есть ли попытки и это сетевая ошибка?
      if (retryCount < 2 && (
        err.message.includes('fetch') ||
        err.message.includes('network')
      )) {
        warn(`⚠️ Повтор обновления продукта ${productId}, попытка ${retryCount + 1}`)

        // Задержка с увеличением: 1 сек, 2 сек
        await delay(1000 * (retryCount + 1))

        // Рекурсивный вызов с увеличенным счетчиком
        return this.updateProductStock(productId, updates, retryCount + 1)
      }

      // Все попытки исчерпаны или не сетевая ошибка
      error(`❌ Ошибка обновления продукта ${productId}:`, err)
      return {
        success: false,
        productId,
        error: err.message
      }
    }
  }

  /**
   * ============================================================
   * МАССОВОЕ ОБНОВЛЕНИЕ ПРОДУКТОВ (Bulk RPC)
   * ============================================================
   * 
   * Синхронизирует все продукты с базой данных за ОДИН запрос
   * Использует PostgreSQL функцию bulk_update_products через RPC
   * 
   * ВЕРСИЯ v2.1 - ОСОБЕННОСТИ:
   * - Не выбрасывает исключение при частичном успехе
   * - Возвращает userMessage для показа пользователю
   * - Улучшенная обработка CORS и сетевых ошибок
   * - Детальное логирование для отладки
   * 
   * ПРОЦЕСС:
   * 1. Валидация входных данных
   * 2. Вызов bulkUpdateProducts() из модуля bulkOperations
   * 3. Анализ результатов (успех/частичный успех/провал)
   * 4. Логирование деталей ошибок
   * 5. Возврат понятного результата для UI
   * 
   * @param {Array} products - Массив продуктов для синхронизации
   *                           Каждый продукт должен содержать:
   *                           - id: number
   *                           - bar1: number
   *                           - bar2: number
   *                           - cold_room: number
   * 
   * @returns {Promise<Object>} Результат синхронизации:
   * {
   *   success: boolean,        // true только если все успешно
   *   total: number,           // Всего продуктов
   *   updated: number,         // Успешно обновлено
   *   failed: number,          // Количество ошибок
   *   errors: Array,           // Детали ошибок [{product_id, error}]
   *   duration: number,        // Время выполнения (мс)
   *   userMessage: string,     // Понятное сообщение для пользователя
   *   hasCORSErrors: boolean   // Были ли CORS ошибки
   * }
   */
  async syncAll(products, availableColumns) {
    // ============================================================
    // ВАЛИДАЦИЯ ВХОДНЫХ ДАННЫХ
    // ============================================================
    
    // Проверка инициализации клиента
    if (!this.client) {
      throw new Error('Supabase клиент не инициализирован')
    }
    
    // Проверка что есть продукты для синхронизации
    if (!products || products.length === 0) {
      log('⚠️ syncAll: нет продуктов для синхронизации')
      return { 
        success: true, 
        total: 0, 
        updated: 0, 
        failed: 0,
        errors: [],
        duration: 0,
        userMessage: 'Нет данных для сохранения',
        hasCORSErrors: false
      }
    }

    log(`🔄 Начало синхронизации ${products.length} продуктов через Bulk RPC...`)
    
    // ============================================================
    // ВЫЗОВ BULK RPC ФУНКЦИИ
    // ============================================================
    
    try {
      // Вызываем функцию массового обновления из модуля bulkOperations
      // Она использует PostgreSQL функцию bulk_update_products
      const result = await bulkUpdateProducts(products, availableColumns)
      
      // ============================================================
      // АНАЛИЗ РЕЗУЛЬТАТА
      // ============================================================
      
      // Логируем краткую статистику
      log('📊 Результат синхронизации:', {
        success: result.success,
        updated: result.updated,
        failed: result.failed,
        total: result.total,
        duration: result.duration
      })
      
      // ВАЖНО: Не выбрасываем исключение даже если есть ошибки
      // Вместо этого возвращаем детальную информацию для UI
      // UI сам решит как показать результат пользователю
      
      // Анализируем тип результата и логируем соответственно
      if (result.success) {
        // Полный успех - все продукты обновлены
        log('✅ Синхронизация завершена успешно')
      } else if (result.updated > 0) {
        // Частичный успех - часть продуктов обновлена
        warn(`⚠️ Частичная синхронизация: ${result.updated}/${result.total}`)
        
        // Проверяем были ли CORS ошибки
        if (result.hasCORSErrors) {
          warn('⚠️ Обнаружены CORS ошибки - возможно проблема с сетью')
        }
      } else {
        // Полный провал - ни один продукт не обновлен
        error('❌ Синхронизация не удалась')
        
        // Проверяем причину провала
        if (result.hasCORSErrors) {
          error('❌ Причина: CORS/сетевые ошибки')
        }
      }
      
      // ============================================================
      // ЛОГИРОВАНИЕ ДЕТАЛЕЙ ОШИБОК
      // ============================================================
      
      // Если есть ошибки - показываем их в консоли
      if (result.errors.length > 0) {
        if (isDev) { console.group('📋 Детали ошибок:') }
        result.errors.forEach((err, index) => {
          error(`${index + 1}.`, err)
        })
        if (isDev) { console.groupEnd() }
      }
      
      // Возвращаем результат для обработки в UI
      return result
      
    } catch (err) {
      // ============================================================
      // ОБРАБОТКА КРИТИЧЕСКИХ ОШИБОК
      // ============================================================

      error('❌ Критическая ошибка синхронизации:', err)

      // Формируем понятный ответ даже при критической ошибке
      // Это гарантирует что UI всегда получит структурированный результат
      return {
        success: false,
        total: products.length,
        updated: 0,
        failed: products.length,
        errors: [{
          error: err.message || 'Unknown error',
          type: 'critical'
        }],
        duration: 0,
        userMessage: `❌ Критическая ошибка: ${err.message || 'Не удалось подключиться к серверу'}`,
        hasCORSErrors: err.message?.toLowerCase().includes('cors') ||
                       err.message?.toLowerCase().includes('network')
      }
    }
  }

}

/**
 * ============================================================
 * ЭКСПОРТ
 * ============================================================
 * 
 * Экспортируем единственный экземпляр API (Singleton pattern)
 * Это гарантирует что во всем приложении используется один клиент
 */
export default new SupabaseAPI()