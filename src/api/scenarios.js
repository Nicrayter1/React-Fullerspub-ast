...
/**
 * ============================================================
 * МАССОВОЕ ОБНОВЛЕНИЕ ФЛАГОВ ПО КАТЕГОРИИ
 * ============================================================
 * 
 ...
 * @returns {Promise<Object>} Результат
 */
export async function updateCategoryFlags(categoryId, flags) {
  try {
    console.log(`🏴 Массовое обновление флагов для категории ${categoryId}:`, flags)
    ...
    return {
      success: true,
      count: data.length,
      message: `Флаги обновлены для ${data.length} продуктов`
    }
